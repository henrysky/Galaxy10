...
def images_url(RA, DEC, scale, height, width, i):
    ...
    str2 = '&ra={:f}&dec={:f}&pixscale={:f}&width={:d}&height={:d}'.format(RA[i], DEC[i], scale, height, width)
    try:
        urllib.request.urlretrieve(str1 + str2, iamge_dir + str(i).rjust(6, '0') + '.jpg')
    except:
        pass
        print(i)

radeg, decdeg = np.genfromtxt(data_table, skip_header=1, usecols=(1, 2), delimiter=",", unpack=True)
